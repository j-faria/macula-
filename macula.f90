...
SUBROUTINE macula(t,ndata,Nspot,mmax,derivatives_int,temporal_int,TdeltaV_int,&
                  Theta_star,Theta_spot,Theta_inst,&
                  Tstart,Tend,&
                  Fmod,dFmod_star,dFmod_spot,dFmod_inst,dFmoddt,deltaratio) bind(c)
...
implicit none
 ...
 ! === INPUTS ===
 REAL(c_double), DIMENSION(ndata), INTENT(IN) :: t
 REAL(c_double), DIMENSION(pstar), INTENT(IN) :: Theta_star
 REAL(c_double), DIMENSION(pspot,Nspot), INTENT(IN) :: Theta_spot
 REAL(c_double), DIMENSION(pinst,mmax), INTENT(IN) :: Theta_inst
 REAL(c_double), DIMENSION(mmax), INTENT(IN) :: Tstart, Tend
 INTEGER(c_int), INTENT(IN) :: derivatives_int, temporal_int, TdeltaV_int
 LOGICAL:: derivatives ! Calculate partial derivatives?
 LOGICAL:: temporal    ! Calculate temporal derivatives?
 LOGICAL:: TdeltaV     ! Calculate transit depth variations?
 ! === VARIABLES ===
 INTEGER :: i, j, k, l, m, n
 INTEGER(c_int), INTENT(IN) :: ndata, mmax, Nspot
 INTEGER :: jmax
 ...
 REAL(c_double), DIMENSION(ndata), INTENT(OUT) :: Fmod, deltaratio
 ! === SECTION 4 ===
 ...
 REAL(c_double), DIMENSION(ndata), INTENT(OUT) :: dFmoddt
 ! === SECTION 9 ===
 REAL(c_double), DIMENSION(ndata,pstar), INTENT(OUT) :: dFmod_star
 REAL(c_double), DIMENSION(ndata,pspot,Nspot), INTENT(OUT) :: dFmod_spot
 REAL(c_double), DIMENSION(ndata,pinst,mmax), INTENT(OUT) :: dFmod_inst
...
!  write(*,*) Theta_star
 ...
 derivatives = derivatives_int == 1
 temporal = temporal_int == 1
 TdeltaV = TdeltaV_int == 1
